...
def get_beta(obs, model):
    ...
    for t in range(n - 2, -1, -1):
        for i in range(N):
            beta[t, i] = sum(tran_model[i, :] * obs_model[:,
                                                          obs[t + 1]] * beta[t + 1, :])
...
#		self.compute_logs()
        ...
        for i in range(self.num_states):
            ...
            for j in range(self.num_states):
                self.transition[i][j] = (
                    self.transition[i][j] + 1.0) / (prefix[i] + self.num_states)
        ...
        for i in range(self.num_states):
            for j in range(self.num_outputs):
                self.observation[i][j] = (
                    self.observation[i][j] + 1.0) / (prefix[i] + self.num_outputs)
